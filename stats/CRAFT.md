# Craft report - engagement by how the reel was CUT

Joined **133** posted reels (queue.json x history.jsonl on permalink).

## Retention - the edit-quality metric

_Share of the reel actually watched. This is the hook test._

| topic | n | median | mean | best |
|---|---|---|---|---|
| utility | 59 | 40% | 41% | 81% |
| talk | 67 | 24% | 25% | 54% |
| meta | 7 | 23% | 24% | 37% |

| duration | n | median | mean | best |
|---|---|---|---|---|
| under 20s | 27 | 46% | 50% | 81% |
| 20-35s | 39 | 31% | 33% | 67% |
| 35-50s | 34 | 27% | 29% | 56% |
| 50-70s | 22 | 17% | 23% | 69% |
| over 70s | 11 | 13% | 13% | 23% |

| fresh/catalog | n | median | mean | best |
|---|---|---|---|---|
| back-catalog | 36 | 41% | 40% | 81% |
| fresh | 97 | 28% | 29% | 69% |

| cover | n | median | mean | best |
|---|---|---|---|---|
| custom cover | 68 | 33% | 35% | 81% |
| default cover | 65 | 28% | 29% | 67% |

| collab | n | median | mean | best |
|---|---|---|---|---|
| collab | 5 | 34% | 38% | 67% |
| solo | 128 | 30% | 32% | 81% |

## By views

| topic | n | median | mean | best |
|---|---|---|---|---|
| utility | 59 | 328.0 | 759.4 | 2761.0 |
| meta | 7 | 152.0 | 163.9 | 289.0 |
| talk | 67 | 116.0 | 123.7 | 1337.0 |

| duration | n | median | mean | best |
|---|---|---|---|---|
| under 20s | 27 | 245.0 | 672.2 | 2337.0 |
| 35-50s | 34 | 159.0 | 453.5 | 2761.0 |
| 50-70s | 22 | 147.0 | 287.4 | 1566.0 |
| over 70s | 11 | 145.0 | 139.8 | 494.0 |
| 20-35s | 39 | 136.0 | 328.6 | 1846.0 |

| fresh/catalog | n | median | mean | best |
|---|---|---|---|---|
| back-catalog | 36 | 260.0 | 784.0 | 2761.0 |
| fresh | 97 | 136.0 | 268.2 | 1841.0 |

| cover | n | median | mean | best |
|---|---|---|---|---|
| default cover | 65 | 153.0 | 473.4 | 2761.0 |
| custom cover | 68 | 141.0 | 345.2 | 2337.0 |

| collab | n | median | mean | best |
|---|---|---|---|---|
| collab | 5 | 194.0 | 467.8 | 1529.0 |
| solo | 128 | 146.0 | 405.5 | 2761.0 |

## By reach

| topic | n | median | mean | best |
|---|---|---|---|---|
| utility | 59 | 275.0 | 612.6 | 2357.0 |
| meta | 7 | 129.0 | 128.3 | 219.0 |
| talk | 67 | 106.0 | 106.0 | 1189.0 |

| duration | n | median | mean | best |
|---|---|---|---|---|
| under 20s | 27 | 207.0 | 520.5 | 1728.0 |
| 35-50s | 34 | 135.5 | 379.4 | 2357.0 |
| 50-70s | 22 | 134.0 | 241.5 | 1314.0 |
| over 70s | 11 | 129.0 | 124.0 | 446.0 |
| 20-35s | 39 | 114.0 | 269.5 | 1554.0 |

| fresh/catalog | n | median | mean | best |
|---|---|---|---|---|
| back-catalog | 36 | 226.0 | 626.1 | 2357.0 |
| fresh | 97 | 116.0 | 222.7 | 1430.0 |

| cover | n | median | mean | best |
|---|---|---|---|---|
| default cover | 65 | 136.0 | 387.4 | 2357.0 |
| custom cover | 68 | 118.5 | 278.8 | 1728.0 |

| collab | n | median | mean | best |
|---|---|---|---|---|
| collab | 5 | 170.0 | 376.6 | 1260.0 |
| solo | 128 | 129.0 | 330.1 | 2357.0 |

## By saves

| topic | n | median | mean | best |
|---|---|---|---|---|
| meta | 7 | 0.0 | 0.0 | 0.0 |
| talk | 67 | 0.0 | 0.2 | 3.0 |
| utility | 59 | 0.0 | 1.3 | 18.0 |

| duration | n | median | mean | best |
|---|---|---|---|---|
| over 70s | 11 | 0.0 | 0.2 | 2.0 |
| 35-50s | 34 | 0.0 | 1.0 | 16.0 |
| 50-70s | 22 | 0.0 | 0.5 | 3.0 |
| 20-35s | 39 | 0.0 | 0.1 | 1.0 |
| under 20s | 27 | 0.0 | 1.5 | 18.0 |

| fresh/catalog | n | median | mean | best |
|---|---|---|---|---|
| fresh | 97 | 0.0 | 0.3 | 5.0 |
| back-catalog | 36 | 0.0 | 1.6 | 18.0 |

| cover | n | median | mean | best |
|---|---|---|---|---|
| default cover | 65 | 0.0 | 0.7 | 16.0 |
| custom cover | 68 | 0.0 | 0.7 | 18.0 |

| collab | n | median | mean | best |
|---|---|---|---|---|
| solo | 128 | 0.0 | 0.7 | 18.0 |
| collab | 5 | 0.0 | 0.0 | 0.0 |

## By likes

| topic | n | median | mean | best |
|---|---|---|---|---|
| utility | 59 | 4.0 | 7.5 | 37.0 |
| meta | 7 | 2.0 | 2.1 | 4.0 |
| talk | 67 | 1.0 | 1.4 | 18.0 |

| duration | n | median | mean | best |
|---|---|---|---|---|
| under 20s | 27 | 3.0 | 6.0 | 31.0 |
| over 70s | 11 | 2.0 | 2.5 | 11.0 |
| 35-50s | 34 | 2.0 | 5.5 | 37.0 |
| 50-70s | 22 | 1.5 | 3.5 | 23.0 |
| 20-35s | 39 | 1.0 | 2.6 | 21.0 |

| fresh/catalog | n | median | mean | best |
|---|---|---|---|---|
| back-catalog | 36 | 3.5 | 7.6 | 37.0 |
| fresh | 97 | 1.0 | 2.9 | 26.0 |

| cover | n | median | mean | best |
|---|---|---|---|---|
| default cover | 65 | 2.0 | 4.9 | 37.0 |
| custom cover | 68 | 1.0 | 3.4 | 26.0 |

| collab | n | median | mean | best |
|---|---|---|---|---|
| collab | 5 | 2.0 | 5.0 | 15.0 |
| solo | 128 | 1.0 | 4.1 | 37.0 |

---

**Read the median, not the mean** - one outlier makes a dead format look alive.

**An empty cell is not a zero** - it is a cut nobody has tried yet.
