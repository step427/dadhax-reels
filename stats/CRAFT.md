# Craft report - engagement by how the reel was CUT

Joined **136** posted reels (queue.json x history.jsonl on permalink).

## Retention - the edit-quality metric

_Share of the reel actually watched. This is the hook test._

| topic | n | median | mean | best |
|---|---|---|---|---|
| utility | 60 | 41% | 41% | 81% |
| talk | 69 | 25% | 26% | 54% |
| meta | 7 | 23% | 24% | 37% |

| duration | n | median | mean | best |
|---|---|---|---|---|
| under 20s | 30 | 46% | 50% | 81% |
| 20-35s | 39 | 31% | 33% | 67% |
| 35-50s | 34 | 27% | 29% | 56% |
| 50-70s | 22 | 17% | 23% | 69% |
| over 70s | 11 | 13% | 13% | 23% |

| fresh/catalog | n | median | mean | best |
|---|---|---|---|---|
| back-catalog | 37 | 41% | 41% | 81% |
| fresh | 99 | 28% | 29% | 69% |

| cover | n | median | mean | best |
|---|---|---|---|---|
| custom cover | 70 | 34% | 35% | 81% |
| default cover | 66 | 28% | 30% | 67% |

| collab | n | median | mean | best |
|---|---|---|---|---|
| collab | 5 | 34% | 38% | 67% |
| solo | 131 | 30% | 32% | 81% |

## By views

| topic | n | median | mean | best |
|---|---|---|---|---|
| utility | 60 | 337.5 | 764.5 | 2764.0 |
| meta | 7 | 152.0 | 163.9 | 289.0 |
| talk | 69 | 120.0 | 124.4 | 1338.0 |

| duration | n | median | mean | best |
|---|---|---|---|---|
| under 20s | 30 | 240.5 | 650.1 | 2340.0 |
| 35-50s | 34 | 159.5 | 453.6 | 2764.0 |
| 50-70s | 22 | 147.0 | 287.4 | 1567.0 |
| over 70s | 11 | 145.0 | 139.8 | 494.0 |
| 20-35s | 39 | 136.0 | 328.6 | 1846.0 |

| fresh/catalog | n | median | mean | best |
|---|---|---|---|---|
| back-catalog | 37 | 268.0 | 791.5 | 2764.0 |
| fresh | 99 | 136.0 | 265.8 | 1841.0 |

| cover | n | median | mean | best |
|---|---|---|---|---|
| default cover | 66 | 155.5 | 477.5 | 2764.0 |
| custom cover | 70 | 141.5 | 344.1 | 2340.0 |

| collab | n | median | mean | best |
|---|---|---|---|---|
| collab | 5 | 194.0 | 467.8 | 1529.0 |
| solo | 131 | 147.0 | 406.6 | 2764.0 |

## By reach

| topic | n | median | mean | best |
|---|---|---|---|---|
| utility | 60 | 283.0 | 615.6 | 2360.0 |
| meta | 7 | 129.0 | 128.3 | 219.0 |
| talk | 69 | 107.0 | 106.6 | 1190.0 |

| duration | n | median | mean | best |
|---|---|---|---|---|
| under 20s | 30 | 189.5 | 503.3 | 1728.0 |
| 35-50s | 34 | 135.5 | 379.5 | 2360.0 |
| 50-70s | 22 | 134.0 | 241.5 | 1314.0 |
| over 70s | 11 | 129.0 | 124.0 | 446.0 |
| 20-35s | 39 | 114.0 | 269.6 | 1554.0 |

| fresh/catalog | n | median | mean | best |
|---|---|---|---|---|
| back-catalog | 37 | 227.0 | 630.6 | 2360.0 |
| fresh | 99 | 116.0 | 220.8 | 1430.0 |

| cover | n | median | mean | best |
|---|---|---|---|---|
| default cover | 66 | 138.0 | 389.8 | 2360.0 |
| custom cover | 70 | 118.5 | 278.1 | 1728.0 |

| collab | n | median | mean | best |
|---|---|---|---|---|
| collab | 5 | 170.0 | 376.6 | 1260.0 |
| solo | 131 | 129.0 | 330.6 | 2360.0 |

## By saves

| topic | n | median | mean | best |
|---|---|---|---|---|
| meta | 7 | 0.0 | 0.0 | 0.0 |
| talk | 69 | 0.0 | 0.2 | 3.0 |
| utility | 60 | 0.0 | 1.3 | 18.0 |

| duration | n | median | mean | best |
|---|---|---|---|---|
| over 70s | 11 | 0.0 | 0.2 | 2.0 |
| 35-50s | 34 | 0.0 | 1.0 | 16.0 |
| 50-70s | 22 | 0.0 | 0.5 | 3.0 |
| 20-35s | 39 | 0.0 | 0.1 | 1.0 |
| under 20s | 30 | 0.0 | 1.4 | 18.0 |

| fresh/catalog | n | median | mean | best |
|---|---|---|---|---|
| fresh | 99 | 0.0 | 0.3 | 5.0 |
| back-catalog | 37 | 0.0 | 1.5 | 18.0 |

| cover | n | median | mean | best |
|---|---|---|---|---|
| default cover | 66 | 0.0 | 0.7 | 16.0 |
| custom cover | 70 | 0.0 | 0.7 | 18.0 |

| collab | n | median | mean | best |
|---|---|---|---|---|
| solo | 131 | 0.0 | 0.7 | 18.0 |
| collab | 5 | 0.0 | 0.0 | 0.0 |

## By likes

| topic | n | median | mean | best |
|---|---|---|---|---|
| utility | 60 | 4.0 | 7.4 | 37.0 |
| meta | 7 | 2.0 | 2.1 | 4.0 |
| talk | 69 | 1.0 | 1.4 | 18.0 |

| duration | n | median | mean | best |
|---|---|---|---|---|
| over 70s | 11 | 2.0 | 2.5 | 11.0 |
| 35-50s | 34 | 2.0 | 5.5 | 37.0 |
| under 20s | 30 | 2.0 | 5.5 | 31.0 |
| 50-70s | 22 | 1.5 | 3.5 | 23.0 |
| 20-35s | 39 | 1.0 | 2.6 | 21.0 |

| fresh/catalog | n | median | mean | best |
|---|---|---|---|---|
| back-catalog | 37 | 3.0 | 7.5 | 37.0 |
| fresh | 99 | 1.0 | 2.8 | 26.0 |

| cover | n | median | mean | best |
|---|---|---|---|---|
| default cover | 66 | 2.0 | 4.9 | 37.0 |
| custom cover | 70 | 1.0 | 3.4 | 26.0 |

| collab | n | median | mean | best |
|---|---|---|---|---|
| collab | 5 | 2.0 | 5.0 | 15.0 |
| solo | 131 | 1.0 | 4.1 | 37.0 |

---

**Read the median, not the mean** - one outlier makes a dead format look alive.

**An empty cell is not a zero** - it is a cut nobody has tried yet.
